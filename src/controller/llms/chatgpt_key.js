...
const addApiKey = (_, key) => {
    ensureTableExists();
    const stmt = database.prepare('INSERT INTO chatgpt_api_keys (key) VALUES  (?)');
    const info = stmt.run(key);
    console.log(`A new api key was successfully added with id ${info.lastInsertRowid}.`);
    return {
...
export { addApiKey, getApiKeys, getDefaultApiKey, removeApiKey, updateDefaultApiKey };
